...
export class ExcelService {
    ...
    async uploadReceivedNoteFile(file: any, storeId: number): Promise<any> {
        const lang = await this.userService.getCurrentLanguage();
        let staffId = 0;
    ...
    async getSalesReport(reportType: number, staffId: number, dateFrom: string = '', dateTo: string = '', storeId = 0): Promise<any> {
        const lang = await this.userService.getCurrentLanguage();
        const data: any = {dateFrom, dateTo, reportType, lang, storeId};
        if (this.staffService.selectedStaff) {
    ...
    async downloadOrderPdf(filename, html: any): Promise<any> {
        const data: any = {html , size: 6};
        return await this.apiService.downloadWithPost(this.userService.apiUrl + '/excel/PdfConvertShare', data, filename);
    }

    async downloadExcelOrderReport(fileName: string, reportType: number, staffId: number,
                                   dateFrom: string = '', dateTo: string = '', storeId = 0): Promise<any> {
        const lang = await this.userService.getCurrentLanguage();
        const data: any = {dateFrom, dateTo, reportType, lang, storeId};
        if (this.staffService.selectedStaff) {
            data.staffId = this.staffService.selectedStaff.id;
