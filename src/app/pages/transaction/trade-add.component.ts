...
export class TradeAddComponent {
    ...
    async ngOnInit(): Promise<void> {
        this.userService.getCurrentCurrency().then((code) => {
    ...
    async showSearchCategory() {
        const modal = await this.modalCtrl.create({ component: CategorySearchComponent });
        await modal.present();
    ...
    addImage(idx: number, isCapture: boolean): void {
        if (!this.platform.is('capacitor')) {
            if (this.lastSamplePictureId === 0) {
                this.doAdd('assets/temp.jpg', idx);
                this.lastSamplePictureId = 1;
            } else if (this.lastSamplePictureId === 1) {
                this.doAdd('assets/baby.jpg', idx);
                this.lastSamplePictureId = 2;
            } else if (this.lastSamplePictureId === 2) {
                this.doAdd('assets/pizza.jpg', idx);
                this.lastSamplePictureId = 0;
            }
            return;
        }
