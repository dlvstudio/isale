...
export class TransferNoteComponent {
    originalNotes: any[];
    notes: any[];
    searchKey = '';
    params: any = null;
    ...
    constructor(
        ...
        private transferNote: TransferNoteService,
        ...
        this.navCtrl.unsubscribe('reloadTransferNoteList', this.reload);
        this.navCtrl.subscribe('reloadTransferNoteList', this.reload);
    }

    async ionViewWillEnter() {
        await this.analyticsService.setCurrentScreen('transfer-note');
    }
    ...
    async onPeriodChanged(data: any): Promise<void> {
        if (data) {
            ...
            this.transferNote.getNotes(this.dateFrom, this.dateTo, this.store ? this.store.id : 0).then(async (notes) => {
                const arr = [];
                if (notes && notes.length) {
                    for (const note of notes) {
                        note.items = JSON.parse(note.itemsJson);
                        note.importStore = note.importStoreId && this.stores && this.stores.length
                            ? this.stores.find(s => s.id == note.importStoreId)
                            : null;
                        note.exportStore = note.exportStoreId && this.stores && this.stores.length
                            ? this.stores.find(s => s.id == note.exportStoreId)
                            : null;
                        arr.push(note);
                    }
                }
                ...
                this.notes = arr;
                this.total = _.sumBy(this.notes, (item: any) => item.amount);
                this.filter();
    ...
    openAdd(): void {
        this.navCtrl.push('/transfer-note/add');
    }

    async search(): Promise<void> {
        this.analyticsService.logEvent('transfer-note-search');
        let notes: any[] = JSON.parse(JSON.stringify(this.originalNotes));
        if (this.searchKey !== null && this.searchKey !== '') {
            ...
            notes = notes.filter((item) => (item.contact && item.contact.fullName && item.contact.fullName.toLowerCase().indexOf(searchKey) !== -1)
                    ...
                    || (item.importStore && item.importStore.name && item.importStore.name.toLowerCase().indexOf(searchKey) !== -1)
                    || (item.exportStore && item.exportStore.name && item.exportStore.name.toLowerCase().indexOf(searchKey) !== -1)
                    || (item.deliverer && item.deliverer.toLowerCase().indexOf(searchKey) !== -1)
                    || (item.deliveryAddress && item.deliveryAddress.toLowerCase().indexOf(searchKey) !== -1)
                    || (item.receiver && item.receiver.toLowerCase().indexOf(searchKey) !== -1)
                    || (item.transportation && item.transportation.toLowerCase().indexOf(searchKey) !== -1)
                    || (item.items && item.items.length
                        && item.items.filter(p => (p.productCode && p.productCode.toLowerCase().indexOf(searchKey) !== -1)
                                ...
                                || (p.barcode && p.barcode.toLowerCase().indexOf(searchKey) !== -1)
    ...
    sortByModifiedAt(notes: any[]): any[] {
        if (notes) {
    ...
    select(id: number): void {
        this.navCtrl.push('/transfer-note/detail', { id });
    }
            ...
            buttons: [
                {
                    ...
                    handler: async () => {
                        ...
                        this.transferNote.delete(note).then(async () => {
                            this.analyticsService.logEvent('transfer-note-delete-success');
                            let i = this.notes.findIndex(item => item.id == note.id);
                            if (i >= 0) {
                                this.notes.splice(i, 1);
                                this.total = _.sumBy(this.notes, (item: any) => item.total);
                            }
    ...
    async deleteRelated(note: any) {
        const arr = [];
        const tradesToDelete = await this.tradeService.getTradesByTransferNote(note.id);
    ...
    exportExcel(): void {
        const data = [];
        data.push([this.translateService.instant('export.transfer-note-report')]);
        ...
        const fileName = 'transfer-note-report-' + Helper.getCurrentDate() + '.xlsx';
    ...
    async presentActionSheet(note: any) {
        const actionSheet = await this.actionSheetCtrl.create({
            ...
            buttons: [
                {
                    text: this.translateService.instant('transfer-note.delete'),
                    role: 'destructive',
                    handler: () => {
                        this.deleteNote(note);
                    }
                }, {
                    text: this.translateService.instant('transfer-note.edit'),
                    handler: () => {
                        this.select(note.id);
                    }
                }, {
    ...
    importNote(): void {
        this.navCtrl.push('/transfer-note/import');
    }
    ...
    doEnteredBarcode = (barcode) => {
        ...
        this.productService.searchByBarcode(barcode).then((product: IProduct) => {
            if (product) {
                this.navCtrl.push('/transfer-note/add', { product: product.id });
                return;
            }
        });
    }
